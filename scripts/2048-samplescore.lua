...
-- 加载字体文件，设置字体样式
font_num = Graphic.LoadFontFromFile("C:\\Windows\\Fonts\\msyh.ttc", 125)
font_score = Graphic.LoadFontFromFile("C:\\Windows\\Fonts\\msyh.ttc", 30)
            ...
            if #grid_list[i] > 1 then
                for j = 1, #grid_list[i] - 1 do
                    if grid_list[i][j].value == grid_list[i][j + 1].value then
                        ...
                        score = score + grid_list[i][j].value
                    end
                end
            end
            -- 再次移除每列中值为 0 网格
            ...
            if #_tmp_grids > 1 then
                for k = 1, #_tmp_grids - 1 do
                    if _tmp_grids[k].value == _tmp_grids[k + 1].value then
                        ...
                        score = score + _tmp_grids[k].value
                    end
                end
            end
            -- 再次移除临时表中值为 0 的网格
...
function CheckGameOver()
    ...
    if _bIsGridsFull then
        ...
        if not _bIsMergeable then
            for i = 1, 3 do
                if grid_list[i][4].value == grid_list[i + 1][4].value then
                    _bIsMergeable = true
                    break
                end
            end
        ...
        if not _bIsMergeable then
            for j = 1, 3 do
                if grid_list[4][j].value == grid_list[4][j + 1].value then
                    _bIsMergeable = true
                    break
                end
            end
...
while not bIsQuit do
    ...
    if _nEndTime - _nStartTime < 1000/20 then
        Time.Sleep(1000/20 - (_nEndTime - _nStartTime))
    end
